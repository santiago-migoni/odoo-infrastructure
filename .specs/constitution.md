...
- **El Makefile es la interfaz operativa para uso manual/humano** (implementado en `008-makefile`, guiado con menús/errores en `012-makefile-ux`) — compartida entre uso manual y CI futuro, nunca lógica de deploy duplicada entre un script y el YAML del pipeline. **Excepción deliberada:** los triggers de `systemd` (timers) invocan los scripts subyacentes directamente, no pasan por `make`/el dispatcher — un trigger desatendido usa el camino más corto con menos piezas que puedan fallar, sin depender de una capa optimizada para enseñar/guiar que no necesita.
- **Ningún comando de Makefile termina en un error mudo de `make`.** Un verbo sin stack (`make down`) muestra un menú de combinaciones válidas; una combinación inválida (`make pull-prod-odoo` sobre un servicio sin imagen oficial) muestra un error que guía al comando correcto. Ninguno de los dos casos ejecuta una acción destructiva. Toda esta lógica de validación/menús/errores vive en un único script (fuente única de verdad), nunca duplicada entre macros de Make y scripts.
- **`down` siempre conserva datos, en todos los stacks, sin excepción.** El borrado deliberado de volúmenes no es un verbo genérico de la matriz — es un comando único con nombre propio (`nuke-staging`) para el único caso real hoy. No existe un verbo `nuke` aplicable a cualquier stack, ni `nuke-prod`.
...
- Targets de Makefile: `<verbo>-<stack>[-<servicio>]` (verbo primero, componente opcional siempre al final — ej. `up-prod`, `down-staging`, `logs-prod-odoo`) — sin variables que memorizar. Verbos de grilla, uniformes en los 5 stacks: `up`, `stop`, `down`, `ps`, `logs`. Verbos de mantenimiento, no uniformes (dependen de si el servicio tiene build propio o imagen oficial): `restart`, `pull`, `rebuild`. Nombres de stack siempre completos (`staging`, `monitoring`, nunca abreviados) — correspondencia 1:1 entre el token del target y el nombre real del artefacto (archivo compose, red, volumen, servicio) en todo el repo.
...
- R06 (2026-07-14): Convención de naming de targets de Makefile invertida, de `<stack>-<service>-<action>` a `<verbo>-<stack>[-<servicio>]` (feature 012, resuelto en sesión de `/grilling` motivada por un fallo real de uso: el usuario no encontró `prod-down` para bajar producción, pese a que el comando ya existía — el problema no era la arquitectura del Makefile sino que era indescubrible). El componente opcional (servicio) siempre va al final, nunca como infijo — habilita refinamiento progresivo de izquierda a derecha. Se establecen verbos de grilla uniformes (`up`/`stop`/`down`/`ps`/`logs`, mismos 5 en todo stack) vs. verbos de mantenimiento no uniformes (`restart`/`pull`/`rebuild`, dependen de si el servicio es build propio o imagen oficial). `down` pasa a significar siempre "conservar datos" sin excepción (antes `staging-down` bajaba y destruía volúmenes bajo el mismo verbo que `prod-down`, que no destruía nada — inconsistencia real que la convención vieja permitía sin decirlo); el borrado deliberado de volúmenes se aísla a un comando con nombre propio (`nuke-staging`), no un verbo genérico de la matriz. Se agrega el principio de que ningún comando termina en un error mudo de `make` (verbo desnudo → menú; combinación inválida → error guiado), implementado con un único script dispatcher como fuente de verdad — no macros de Make + scripts duplicando la misma validación. Excepción explícita: `systemd` sigue invocando los scripts subyacentes directo, nunca el dispatcher, decidido deliberadamente para que un trigger desatendido no dependa de una capa optimizada para guiar/enseñar que no necesita.
