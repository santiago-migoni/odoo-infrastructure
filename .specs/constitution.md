...
- **Aplicación**: Odoo 19 Community, imagen propia `FROM odoo:19.0` + addons custom (submódulo git separado) + estructura lista para OCA/Enterprise
...
- **Backups**: contenedor efímero propio (`postgres:19-alpine` + `rclone` + `gnupg`), destino Cloudflare R2 + copia local
...
- **El Makefile es la única interfaz operativa**, compartida entre uso manual y CI — nunca lógica de deploy duplicada entre un script y el YAML del pipeline.
...
- **Nunca usar el tag `latest`** de ninguna imagen en producción — siempre versión mayor fija (`odoo:19.0`) + tag de build por commit SHA.
...
- Imágenes Docker: tag de versión mayor (`odoo:19.0`) + tag de build por commit SHA, nunca `latest`
