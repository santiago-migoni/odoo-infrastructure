...
- [x] VERIFY US4 — `systemd`/`systemctl` no disponibles en macOS (este entorno de desarrollo) para `systemd-analyze verify`; revisado a mano contra la sintaxis INI estándar de `systemd` (`[Unit]`/`[Service]`/`[Timer]`/`[Install]`, `Type=oneshot`, `OnCalendar=daily`, `WantedBy=timers.target`) — sintácticamente correcto, pero sin validación automatizada real. Limitación de la plataforma de desarrollo, a confirmar en el servidor real (Linux) antes de considerar esto cerrado del todo
