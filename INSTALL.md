...
**Siempre rebuildear después de cambiar de tag.** `docker compose up -d` nunca reconstruye una imagen por sí solo — si el `Dockerfile` cambió entre releases pero la imagen local (`odoo-infrastructure-prod-odoo`) ya existe con ese nombre, sigue usando la vieja hasta que se la reconstruya explícitamente. Rebuildear con `--no-cache` y confirmar que el `CMD`/`ENTRYPOINT` quedaron como espera el `Dockerfile` actual, antes de asumir que el problema es otra cosa:

```bash
docker compose -f prod/docker/docker-compose.yml build --no-cache odoo
docker inspect --format 'ENTRYPOINT={{.Config.Entrypoint}} CMD={{.Config.Cmd}}' odoo-infrastructure-prod-odoo
...
```bash
docker build -f prod/docker/Dockerfile -t odoo-prod:$(git rev-parse --short HEAD) .
```
...
```bash
cp monitoring/env/.env.monitoring.example monitoring/env/.env.monitoring   # completar con credenciales reales (el password de MONITORING_DB_PASSWORD va embebido en el DSN de DATA_SOURCE_NAME, no como variable separada; también SMTP y OPERATOR_EMAIL), nunca commitear
```
...
docker image rm odoo-infrastructure-prod-odoo odoo-infrastructure-staging-odoo-staging odoo-infrastructure-backup-backup 2>/dev/null || true
